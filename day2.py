def run(map, noun, verb, finalValuePosition, log_level):
  map[1] = noun
  map[2] = verb
  ...
  position = 0
  while process(map, position, log_level) == 0:
    position += 4
...
def process(map, position, log_level):
  opcode = map[position]
  ...
  if opcode == 1:
    add(map, map[position + 1], map[position + 2], map[position + 3], log_level)
  elif opcode == 2:
    mul(map, map[position + 1], map[position + 2], map[position + 3], log_level)
  else:
    if opcode != 99:
...
def add(map, posA, posB, target, log_level):
...
def mul(map, posA, posB, target, log_level):
  value = map[posA] * map[posB]
  map[target] = value
